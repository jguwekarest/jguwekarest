package org.kramerlab.wekarestapi;

public class Constants {

    public static final String SAVE_MODEL_NOTE = "Returns a Task URI.";
    public static final String SAVE_DATASSET_NOTE = "Save the dataset by sending the Accept header text/uri-list as content-type. Use text/x-arff to preview the data.";
    //public static final String SAVE_DATASSET_FILTER_NOTE = "Save to a new dataset by POST request the content-type text/uri-list.";

    public static final String TEXT_ARFF = "text/x-arff";
    public static final String TEXT_URILIST ="text/uri-list";

}
