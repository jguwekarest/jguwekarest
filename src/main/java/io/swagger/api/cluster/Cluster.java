...
public class Cluster {
    ...
    public Response clusterSimpleKMeansPost(
        ...
        @FormDataParam("file") FormDataContentDisposition fileDetail,
        @Parameter(description = "Dataset URI or local dataset ID (to the arff representation of a dataset).",
            schema = @Schema(defaultValue = ""))@FormDataParam("datasetUri") String datasetUri,
        ...
        @Parameter(description = "If using canopy clustering for initialization and/or speedup this is the minimum T2-based density below which a canopy will be pruned " +
        ...
        @Parameter(description = "The T2 distance to use when using canopy clustering. Values < 0 indicate that this should be set using a heuristic based " +
        ...
        @Parameter(description = "The initialization method to use. Random(0), k-means++(1), Canopy(2) or farthest first(3).",
            schema = @Schema(defaultValue = "0",allowableValues = {"0","1","2","3"}))@FormDataParam("initializationMethod") Integer initializationMethod,
    ...
    public Response clusterHierarchicalPost(
        ...
        @Parameter(description = "Dataset URI or local dataset ID (to the arff representation of a dataset).",
            schema = @Schema(defaultValue = ""))@FormDataParam("datasetUri") String datasetUri,
        @Parameter(description = "Flag to indicate whether the cluster should be print in Newick format. This can be useful for display in other programs. " +
            ...
            schema = @Schema(defaultValue = ""))@FormDataParam("printNewick") Boolean printNewick,
        @Parameter(description = "Sets the number of clusters. If a single hierarchy is desired, set this to 1.",
            schema = @Schema(defaultValue = "1"))@FormDataParam("numClusters") Integer numClusters,
        @Parameter(description = "Sets the method used to measure the distance between two clusters.",
            schema = @Schema(allowableValues = {"SINGLE","COMPLETE","ADJCOMPLETE","AVERAGE","MEAN","CENTROID","WARD","NEIGHBOR_JOINING"}, defaultValue = "SINGLE"))
            @FormDataParam("linkType") String linkType,
        @Parameter(description = "If set to false, the distance between clusters is interpreted as the height of the node linking the clusters. " +
            ...
            schema = @Schema(defaultValue = ""))@FormDataParam("distanceIsBranchLength") Boolean distanceIsBranchLength,
        @Parameter(description = "Sets the distance function, which measures the distance between two individual. " +
           ...
           schema = @Schema(defaultValue = ""))@FormDataParam("distanceFunction") String distanceFunction,
        @Parameter(description = "authorization token") @HeaderParam("subjectid") String subjectid,
        @Context UriInfo ui, @Context HttpHeaders headers, @Context SecurityContext securityContext)
        throws Exception {
            HashMap<String, Object> params = new HashMap<>();
